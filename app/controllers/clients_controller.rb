class ClientsController < ApplicationController
    ...
    respond_to do |format|
      ...
      format.turbo_stream do
        ...
        if params[:search].present? || params[:filter].present?
          render turbo_stream: turbo_stream.update('clients_content', partial: 'clients_list',
  ...
  def create
    ...
    respond_to do |format|
      if @client.save
        ...
        format.turbo_stream do
          render turbo_stream: turbo_stream.update('new_client_form', partial: 'clients/form',
  ...
  def update
    respond_to do |format|
      if @client.update(client_params)
        ...
        format.turbo_stream do
          render turbo_stream: turbo_stream.update('edit_client_form', partial: 'clients/form',
                                                                       locals: { client: @client })
        end
        format.html do
  ...
  def create_measurement
    ...
    respond_to do |format|
      if @measurement.save
        ...
        format.turbo_stream do
          render turbo_stream: turbo_stream.update('measurement_form', partial: 'clients/measurement_form',
  ...
  def create_payment
    ...
    respond_to do |format|
      if @payment.save
        ...
        format.turbo_stream do
          render turbo_stream: turbo_stream.update('payment_form', partial: 'clients/payment_form',
  ...
  def create_skinfold
    ...
    respond_to do |format|
      if @skinfold.save
        ...
        format.turbo_stream do
          render turbo_stream: turbo_stream.update('skinfold_form', partial: 'clients/skinfold_form',
                                                                    locals: { skinfold: @skinfold })
        end
      ...
      if @client.destroy
        ...
        format.turbo_stream do
          render turbo_stream: [
            ...
            turbo_stream.update('flash_messages', partial: 'layouts/flash',
                                                  locals: { flash: { success: 'Cliente deletado com sucesso' } }),
        ...
        format.turbo_stream do
          render turbo_stream: turbo_stream.update(
            ...
            locals: { flash: { danger: 'Erro ao deletar cliente' } }
          )
        end
        format.html do
  ...
  def measurement_params
    params.expect(measurement: %i[height weight chest left_arm right_arm waist abdomen hips
                                  left_thigh righ_thigh])
  end
  ...
  def payment_params
    params.expect(payment: %i[payment_date value])
  end

  def skinfold_params
    params.expect(skinfold: %i[chest midaxilary subscapular bicep tricep abdominal suprailiac
                               thigh calf])
  end

  def require_same_user
    return unless current_user != @client.user && !current_user.admin?
  ...
  def prepare_measurements_chart_data
    ...
    {
      labels: measurements_data.map { |m| m.created_at&.strftime('%d/%m') },
  ...
  def prepare_skinfolds_chart_data
    ...
    {
      labels: skinfolds_data.map { |s| s.created_at&.strftime('%d/%m') },
      ...
      sum: skinfolds_data.map do |s|
        tricep = s.tricep.to_f
        subscapular = s.subscapular.to_f
        suprailiac = s.suprailiac.to_f
